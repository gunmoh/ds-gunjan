...
-- Create the weekly winner dma table . Change the where create_timestamp to choose which table to smooth

DROP TABLE IF EXISTS gunjanmohan.dma_mapping_smoothing;
CREATE TABLE gunjanmohan.dma_mapping_smoothing AS (
...
select tloc.fk_location_id, loc.zipcode, loc.fk_dma_id, loc.region,
       ...
       case when next_create_timestamp = '2100-01-01 00:00:00.000000'
        ...
        --Using the new updated location table 
        left join gunjanmohan.location_upd loc
...
-- Create the new tv_geolocation table

DROP TABLE IF EXISTS gunjanmohan.tv_geolocation_smooth;
CREATE TABLE  gunjanmohan.tv_geolocation_smooth AS (
SELECT
 ...
 map.week_start, map.week_end, map.time_spent as smooth_dma_time,  map.fk_dma_id,
 map.dma_code as smooth_dma, map.assigned_dma_name, map.zipcode as smooth_zip
FROM detection.tv_geolocation tloc
  left JOIN gunjanmohan.dma_mapping_smoothing map ON tloc.fk_tvid = map.fk_tvid
WHERE tloc.create_timestamp <= map.week_end
  AND tloc.next_create_timestamp >= map.week_start
  -- Change create timestamp to edit 
...
select count(distinct fk_tvid) as total_tvs,
       ...
       (switched_dma *1.0)/total_tvs as perc_dma_switch
        from (
select *,
      -- case when nvl(fk_dma_id,0) <> nvl(next_dma,0) or (nvl(fk_dma_id,0) <> nvl(next_dma,0) and next_time is not null ) then 1
           --else 0 end as dma_sflag
...
select fk_tvid, create_timestamp, next_create_timestamp, smooth_dma as fk_dma_id,
       ...
       DATEDIFF(minute, create_timestamp, next_create_timestamp) AS minutes_diff
        from gunjanmohan.tv_geolocation_smooth
        order by 1,2,3 )a
order by 1,2,3
    ) b
